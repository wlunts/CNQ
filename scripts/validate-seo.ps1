...
# 7b. (2026-09-21 removed: case-article body-link mandatory check dropped under natural-compliance rule.
#     Previously required /services#inspection (mandatory) + /about or / (home) — now governed by
#     "natural compliance" judgment (read-flow / no hard-sell / same-URL-once-per-article).)

if ($clusterIssues -eq 0) {
    Write-Host "  [OK] All factory articles link to pillar page (/services/china-factory-audit)" -ForegroundColor Green
}
