$files = Get-ChildItem -Path (Split-Path -Parent $PSScriptRoot) -Filter *.html -Recurse | Where-Object { $_.FullName -notlike '*\Upload\*' -and $_.FullName -notlike '*\.git\*' -and $_.FullName -notlike '*\node_modules\*' } | Sort-Object FullName
...
foreach ($f in $files) {
    ...
    $rel = $f.FullName.Replace(((Split-Path -Parent $PSScriptRoot) + '\'),'').Replace('\','/')
